...
function extraerDinero (cuenta) {
  document.getElementById('extraer-btn').addEventListener('click', () => {
    ...
    while (
      cantidad > cuenta.saldoCuenta ||
      cantidad > cuenta.limiteExtraccion
    ) {
      cantidad = prompt(
        'Saldo actual de :' +
          cuenta.saldoCuenta +
          'ingrese un monto valido dentro de los limites de extracion'
      )
    }
...
function iniciarSesion () {
  ...
  for (var i = 0; i < cuentas.length; i++) {
    if (
      accountNumber == cuentas[i].numeroCuenta &&
      pass == cuentas[i].codigoSeguridad
    ) {
      user = cuentas[i]
      return user
    }
...
function actualizarLimiteEnPantalla (limite) {
  document.getElementById('limite-extraccion').innerHTML =
    'Tu límite de extracción es: $' + limite
}
